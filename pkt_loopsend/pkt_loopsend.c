...
static void lcore_main(void)
{
    ...
    while (!force_quit/* && loop_count < 1000*/) {
        ...
        // Transmit packets only when receive packets 
        int i;
        uint16_t nb_tx = 0;
        for(i = 0; i < BURST_SIZE; i++){
            bufs_tx[i] = make_testpkt();
        }
        if(total_tx-total_rx < 1000){
            nb_tx = rte_eth_tx_burst(enabled_port, 0, bufs_tx, BURST_SIZE);
        }
        // if(total_tx-total_rx >= 999 ){
        //     printf("loop %ld: send packets %d, total sent %ld packets, receive %ld packets, the gap is %ld\n", loop_count, nb_tx, total_tx, total_rx, (total_tx-total_rx));
        ...
        //Receive packets
        const uint16_t nb_rx = rte_eth_rx_burst(enabled_port, 0, bufs_rx, BURST_SIZE);
        total_rx += nb_rx;
        if (nb_rx > 0){
