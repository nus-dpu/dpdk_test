...
def pktfile_gen(file, flow_num, pkts_count, pkts_per_write, zipf_a, flow_group):
    ...
    for i in range(pkts_count):
        ...
        src_ip = socket.inet_ntoa(struct.pack("!I", (SRC_IP_PREFIX) + (zipf_dist_int[i] % flow_group)))
        dst_ip = socket.inet_ntoa(struct.pack("!I", (DST_IP_PREFIX) + (int)(zipf_dist_int[i] / flow_group)))
...
def main():
    ...
    pktfile_gen(filename, flow_num, pkt_num, packets_per_write, zipf_para, flow_group)
