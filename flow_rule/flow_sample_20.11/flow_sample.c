...
generate_flow_rule(double *time_list, struct rte_flow ** flows){
	...
	for(i = 0; i <  FLOW_NUM; i++){
		...
		// 							  1234, 5678,
		// 							  &error);
		flow = generate_ipv4_sample_flow(port_id, 0, SRC_IP, FULL_MASK, 
		                          DEST_IP_PREFIX + i, FULL_MASK, 
								  &error);
...
static void
query_flow_rule(double *time_list, struct rte_flow ** flows, uint32_t flow_num){
	struct rte_flow_error error;
	int i;
	...
	for(i = 0 ; i < flow_num; i++){
		...
		uint64_t start = rte_rdtsc();
		ret = rte_flow_query(port_id, flows[i], action, &query, &error);
		...
		if (unlikely(ret)) {
			printf("Failed to query flow %d, for port %u, err msg: %s\n", i, port_id, error.message);
		}
		else{
			printf("query info: reset, %d; hits_set, %d; bytes_set, %d; reserved, %d; hits, %ld, bytes, %ld\n", \
			       query.reset, query.hits_set, query.bytes_set, query.reserved, query.hits, query.bytes);
		}
	}
}
...
packet_send_main_loop(uint32_t lcore_id)
{
	...
	while (!force_quit && total_rx < PKT_NUM) {
		uint16_t i, j;
