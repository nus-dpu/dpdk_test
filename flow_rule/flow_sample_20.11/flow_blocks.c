...
#define IPV4_PATTERN_SAMPLE_NUM      3
#define IPV4_ACTION_SAMPLE_NUM       4
#define ACTION_FOR_SAMPLE_NUM       2
...
struct rte_flow *
generate_ipv4_sample_flow(uint16_t port_id, uint16_t rx_q,
		uint32_t src_ip, uint32_t src_mask,
		uint32_t dest_ip, uint32_t dest_mask,
		struct rte_flow_error *error);
struct rte_flow *
		...
		struct rte_flow_error *error)
{
	/* Declaring structs being used. 8< */
	struct rte_flow_attr attr;
	struct rte_flow_item pattern[IPV4_PATTERN_SAMPLE_NUM];
	struct rte_flow_action action[IPV4_ACTION_SAMPLE_NUM];
	struct rte_flow_action action_sample[ACTION_FOR_SAMPLE_NUM];
	...
	memset(pattern, 0, sizeof(pattern));
	memset(action, 0, sizeof(action));
	memset(action_sample, 0, sizeof(action_sample));
	 ...
	 * one action only,  move packet to queue
	 */
	action[0].type = RTE_FLOW_ACTION_TYPE_SAMPLE;
	action[0].conf = &sample;
	action[1].type = RTE_FLOW_ACTION_TYPE_QUEUE;
	action[1].conf = &queue;
	action[2].type = RTE_FLOW_ACTION_TYPE_COUNT;
	action[2].conf = &counter;
