...
static void lcore_main(uint32_t lcore_id)
{
    ...
    FILE *fp;
	double add_time_all = 0;
	uint64_t start_all = 0;
    ...
    while (!force_quit) {
		...
		for(i = 0 ; i < TEST_FLOW_NUM; i++){
			double rte_time = 0;
			// uint64_t start = rte_rdtsc();
			flow = generate_ipv4_udp_flow(lconf->port, 0, SRC_IP, FULL_MASK, 
											DEST_IP_PREFIX + i + BASE_FLOW_NUM + i*lconf->vid, FULL_MASK, 
		...
		double add_time_all=(double)(rte_rdtsc() - start) / rte_get_timer_hz();
    }
...
//print add_time_all
    ...
    //     fprintf(fp, "base_flow_num, test_flow_num, core_id, rte_create_time\r\n");
    // }else{
    //     fp = fopen(AVG_TIME_FILE, "a+");
    // }
	// if( fp == NULL ){
	...
	// fprintf(fp,"%d,%d,%d,%lf\r\n", BASE_FLOW_NUM, TEST_FLOW_NUM, lconf->vid, add_time_all);
	// fclose(fp);
	APP_LOG("%d core add 100 flows using %lf time", lconf->vid, add_time_all);
}
...
int main(int argc, char *argv[])
{
	...
	for(i = 0 ; i < BASE_FLOW_NUM; i++){
		double rte_time = 0;
		flow = generate_ipv4_udp_flow(enabled_port, 0, SRC_IP, FULL_MASK, 
