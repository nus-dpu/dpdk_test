...
int printlog(struct output_data *data){
    ...
    if (unlikely(access(OUTPUT_FILE, 0) != 0)){
        ...
        fprintf(fp, "flow_num, percentage, quantile, run_time\r\n");
    }else{
        fp = fopen(OUTPUT_FILE, "a+");
    }
    
    fprintf(fp, "%lu,",data->flow_num);
    fprintf(fp, "%f,",data->percentage);
    fprintf(fp, "%d," ,data->quantile);
    fprintf(fp, "%f\r\n", data->run_time);
}
...
int main(int argc, char *argv[]) {
    ...
    // double run_time = (double) cpu_cycles / rte_get_timer_hz();
    double run_time = (double) cpu_cycles / CPU_HZ;
    printf("%f quantile is %d, calculate time is %fs\n", QUANTILE_PER*100, quantile, run_time);

    struct output_data data = {
        .flow_num = flow_num,
        .percentage = QUANTILE_PER,
        .quantile = quantile,
        .run_time = run_time
    };
